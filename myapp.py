...
# Refresh function
# -------------------------------
def get_favourites_with_lowest_promo_price():
    """Check if favourites are at their lowest promo price."""
    ...
    for fav in favourites:
        ...
        # Look up the current promo price in the Products table
        product = next((p for p in products if p["uri"] == uri), None)
        if not product:
            continue
        current_promo_price = product.get("raw_ec_promo_price", "N/A")
        if current_promo_price == "N/A":
            continue

        # Look up the lowest promo price in the Price History table
        history = [entry for entry in price_history if entry["URI"] == uri and entry["Price"] != "N/A"]
        if not history:
            continue
        lowest_promo_price = min(float(entry["Price"]) for entry in history)

        # Compare prices
        if float(current_promo_price) == lowest_promo_price:
            lowest_price_items.append({
                "Title": product.get("title", "Unknown"),
                "URI": uri,
                "Current Promo Price": current_promo_price,
                "Lowest Promo Price": lowest_promo_price
            })
    return lowest_price_items

def send_email_with_lowest_promo_prices(items):
    """Send an email with the list of favourite items at their lowest promo price using Postmark SMTP."""
    if not items:
        return
    ...
    subject = "Favourites at Their Lowest Promo Price"
    ...
    html_content = "<h3>Favourites at Their Lowest Promo Price</h3><table border='1'><tr><th>Title</th><th>URI</th><th>Current Promo Price</th><th>Lowest Promo Price</th></tr>"
    for item in items:
        html_content += f"<tr><td>{item['Title']}</td><td><a href='{item['URI']}'>{item['URI']}</a></td><td>${item['Current Promo Price']}</td><td>${item['Lowest Promo Price']}</td></tr>"
    html_content += "</table>"
...
def background_update(df_products, today_str):
    ...
    # Check favourites for lowest promo prices and send an email
    lowest_price_items = get_favourites_with_lowest_promo_price()
    send_email_with_lowest_promo_prices(lowest_price_items)
...
def refresh_data(store_id=None):
   ...
   #     return load_products_from_supabase()
    ...
    if store_id:
        initial_payload["dictionaryFieldContext"] = {
             "stores_stock": "",
    ...
    for idx, row in page_data.iterrows():
        ...
        if pd.notna(thumbnail_url) and thumbnail_url != 'N/A':
            ...
            with st.popover("Enlarge Image"):
                ...
                st.image(large_image_url, width=True)
        else:
